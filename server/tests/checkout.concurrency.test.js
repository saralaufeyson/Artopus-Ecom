...
test('Concurrent checkouts do not oversell stock', async () => {
  ...
  const p = await request(app).post('/api/products').set('Authorization', `Bearer ${adminToken}`).send({ type: 'merchandise', title: 'Concurrent', description: 'x', price: 1, category: 'Test', stockQuantity: 1, imageUrl: 'http://x.jpg' });
  ...
  const ci1 = await request(app).post('/api/payments/create-intent').set('Authorization', `Bearer ${t1}`).send({ items: [{ productId: prod._id, quantity: 1 }], shippingAddress: { street: 'A', city: 'B', state: 'C', zip: '1', country: 'X' } });
  ...
  const ci2 = await request(app).post('/api/payments/create-intent').set('Authorization', `Bearer ${t2}`).send({ items: [{ productId: prod._id, quantity: 1 }], shippingAddress: { street: 'A', city: 'B', state: 'C', zip: '1', country: 'X' } });
