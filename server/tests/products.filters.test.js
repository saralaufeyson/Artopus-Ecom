...
test('Product filters (type and full-text q) work as expected', async () => {
  ...
  const a = await request(app).post('/api/products').set('Authorization', `Bearer ${adminToken}`).send({
    type: 'merchandise', title: 'Sticker A', description: 'Cute sticker', price: 2.5, category: 'Sticker', imageUrl: 'http://example.com/1.jpg',
  });
  expect(a.status).toBe(201);

  // Create original artwork
  const b = await request(app).post('/api/products').set('Authorization', `Bearer ${adminToken}`).send({
    type: 'original-artwork', title: 'Unique Art', description: 'One-of-a-kind', price: 100, category: 'Painting', imageUrl: 'http://example.com/2.jpg',
  });
