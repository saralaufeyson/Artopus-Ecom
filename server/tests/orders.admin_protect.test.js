...
test('Non-admin cannot update order status', async () => {
  ...
  // Create product
  const prod = await request(app).post('/api/products').set('Authorization', `Bearer ${adminToken}`).send({
    type: 'merchandise', title: 'Book', description: 'Nice', price: 10, category: 'Book', stockQuantity: 10, imageUrl: 'http://example.com/book.jpg',
  });
  ...
  const ci = await request(app).post('/api/payments/create-intent').set('Authorization', `Bearer ${token}`).send({
    items: [{ productId: prod.body._id, quantity: 1 }],
    shippingAddress: {
